...
# I think this mostly tests your algo organization.
...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
      ...
      for domain in cpdomains:
        visits, url = domain.split(' ')
        visits = int(visits)
        subdomains = url.split('.')

        for i, subdomain in enumerate(subdomains):
          url = '.'.join(subdomains[i:])
          counts[url] = counts.get(url, 0) + visits

      return [f"{counts[subdomain]} {subdomain}" for subdomain in counts]
